...
[Serializable]
public class MuscleWeights {
    public Transform muscleRef;
    ...
    public MuscleWeights(Transform muscleRef) {
        this.muscleRef = muscleRef;
        mappingWeight = 1f;
        this.pinWeight = 1f;
        muscleWeight = 1f;
    }
...
namespace RootMotion.Dynamics {
    ...
    public partial class PuppetMaster : MonoBehaviour {
        ...
        [Tooltip("Ignore muscle weights in favor of built-in values")]
        public bool useGlobalWeights;

        [Tooltip("My personal atrocity against fine code.")]
        /// <summary>
        /// The Muscles managed by this PuppetMaster.
        /// </summary>
        public MuscleWeights[] musclesWeights = new MuscleWeights[0];

        //private readonly Dictionary<Muscle, MuscleWeights> _weightsMap = new();
        ...
        public bool isActive {
            get { return gameObject.activeInHierarchy && initiated && (activeMode == Mode.Active || isBlending); }
        }
        ...
        public UpdateMode updateMode {
            get {
                return targetUpdateMode == AnimatorUpdateMode.AnimatePhysics
        ...
        public bool controlsAnimator {
            get { return isActiveAndEnabled && isActive && initiated && updateMode == UpdateMode.FixedUpdate; }
        }
        ...
        public bool isBlending {
            get { return isSwitchingMode || isSwitchingState; }
        }
...
*/
        ...
        private void Initiate() {
            ...
            for (int i = 0; i < muscles.Length; i++) {
                ...
                if (muscles[i].jointBreakBroadcaster == null)
                    muscles[i].jointBreakBroadcaster =
                        muscles[i].joint.gameObject.AddComponent<JointBreakBroadcaster>();
        ...
        public void OnPreSimulate(float deltaTime) {
            ...
            if (!isFrozen) {
                ...
                */
                ...
                // Update Muscles
                for (int i = 0; i < muscles.Length; i++) {
                    float muscleWeightC = muscleWeight;
                    float pinWeightC = pinWeight;
                    if (!useGlobalWeights)
                        for (int j = 0; j < musclesWeights.Length; j++) {
                            if (musclesWeights[j].muscleRef == muscles[i].target) {
                                muscleWeightC = Mathf.Clamp01(musclesWeights[j].muscleWeight * mappingBlend);
                                pinWeightC = musclesWeights[j].pinWeight;
                                break;
                            }
                        }

                    Debug.Log($"Pin Weight C: {pinWeightC}");
                    muscles[i].Update(pinWeightC, muscleWeightC, muscleSpring, muscleDamper, pinPow,
                        pinDistanceFalloff,
                        true, angularPinning, deltaTime);
                }
            }
                        ...
                        if (isActive) {
                            for (int i = 0; i < muscles.Length; i++) {
                                if (!useGlobalWeights)
                                    for (int j = 0; j < musclesWeights.Length; j++) {
                                        if (musclesWeights[j].muscleRef == muscles[i].target) {
                                            mW = Mathf.Clamp01(musclesWeights[j].mappingWeight * mappingBlend);
                                            break;
                                        }
                                    }
            ...
            if (!isFrozen) {
                ...
                */
                ...
                for (int i = 0; i < muscles.Length; i++) {
                    ...
                    muscles[i].Update(pinWeightC, muscleWeightC, muscleSpring, muscleDamper, pinPow,
                        pinDistanceFalloff,
                        true, angularPinning, Time.fixedDeltaTime);
                }
            }
        ...
        private void MoveToTarget() {
            if (PuppetMasterSettings.instance == null || (PuppetMasterSettings.instance != null &&
                                                          PuppetMasterSettings.instance.UpdateMoveToTarget(this))) {
                foreach (Muscle m in muscles) {
                    m.MoveToTarget();
                }
            }
        ...
        private void Read() {
            ...
            if (teleport) {
                ...
                Quaternion targetDeltaRotation =
                    RootMotion.QuaTools.FromToRotation(targetRoot.rotation, teleportRotation);
                ...
                muscles[0].targetMappedPosition = p + targetDeltaRotation * (muscles[0].targetMappedPosition - p) +
                                                  targetDeltaPosition;
                muscles[0].targetSampledPosition = p + targetDeltaRotation * (muscles[0].targetSampledPosition - p) +
                                                   targetDeltaPosition;
                ...
                foreach (BehaviourBase behaviour in behaviours)
                    behaviour.OnTeleport(targetDeltaRotation, targetDeltaPosition, p, teleportMoveToTarget);
        ...
        private AnimatorUpdateMode targetUpdateMode {
            get {
                ...
                if (targetAnimation != null)
                    return targetAnimation.animatePhysics
        ...
        [ContextMenu("Custom/Set Muscle Weight Refs")]
        public void SetMusclesWeightsRefs() {
            int musclesLength = muscles.Length;
            musclesWeights = new MuscleWeights[musclesLength];
            for (int i = 0; i < musclesLength; i++) {
                Transform muscleRef = muscles[i].target;
                musclesWeights[i] = new MuscleWeights(muscleRef);
            }
        }

        /// <summary>
        /// Контроллирует паппет во время телепорта
        /// </summary>
        /// <param name="b">False - начало телепорта, True - конец телепорта</param>
        public void TeleportSequence(bool b) {
            mode = b ? Mode.Active : Mode.Kinematic;
            blendTime = b ? 0.1f : 0f;
        }
    }
}