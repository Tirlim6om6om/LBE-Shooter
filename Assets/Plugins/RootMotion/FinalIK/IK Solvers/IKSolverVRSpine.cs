    ...
    public partial class IKSolverVR : IKSolver {
        ...
        public class Spine : BodyPart {
            ...
            internal VirtualBone pelvis {
                get { return bones[pelvisIndex]; }
            }

            internal VirtualBone firstSpineBone {
                get { return bones[spineIndex]; }
            }
            ...
            internal VirtualBone head {
                get { return bones[headIndex]; }
            }

            private VirtualBone neck {
                get { return bones[neckIndex]; }
            }
            ...
            //public float stretchingMlp;
            private AnimationCurve stretchCurve = new AnimationCurve();

            protected override void OnRead(Vector3[] positions, Quaternion[] rotations, bool hasChest, bool hasNeck,
                bool hasShoulders, bool hasToes, bool hasLegs, int rootIndex, int index) {
            ...
            public override void PreSolve(float scale) {
                ...
                if (useAnimatedHeadHeightWeight > 0f && useAnimatedHeadHeightRange > 0f) {
                    ...
                    if (animatedHeadHeightBlend > 0f) {
                        float headTargetVOffset =
                            V3Tools.ExtractVertical(IKPositionHead - head.solverPosition, rootUp, 1f).magnitude;
                        ...
                        IKPositionHead += V3Tools.ExtractVertical(head.solverPosition - IKPositionHead, rootUp,
                            useAnimatedHeadHeightWeight);
                    }
                }
            ...
            public override void ApplyOffsets(float scale) {
                ...
                if (pelvisRotationWeight <= 0f) anchorRotation = headRotation * anchorRelativeToHead;
                    ...
                    anchorRotation = Quaternion.Lerp(headRotation * anchorRelativeToHead,
                        pelvisRotation * anchorRelativeToPelvis, pelvisRotationWeight);
            ...
            public void Solve(Animator animator, VirtualBone rootBone, Leg[] legs, Arm[] arms, float scale) {
                ...
                Bend(bones, pelvisIndex, chestIndex, chestTargetRotation, chestRotationOffset, chestClampWeight, false,
                    neckStiffness * rotationWeight);

                if (LOD < 1 && chestGoalWeight > 0f) {
                    Quaternion c = Quaternion.FromToRotation(bones[chestIndex].solverRotation * chestForward,
                    ...
                    Bend(bones, pelvisIndex, chestIndex, c, chestRotationOffset, chestClampWeight, false,
                        chestGoalWeight * rotationWeight);
                }
            ...
            private void Stretching() {
                ...
                float bodyLength = Vector3.Distance(bones[0].solverPosition, bones[^1].solverPosition);

                float spineLength = 0;
                Vector3 mAdd = Vector3.zero;
                Vector3 delta = Vector3.zero;
       

                /*
                 *  Временно отключено. Причина - вращающийся пелвис при калибровке
                 * //stretchingMlp = 1f;
                for (int i = 0; i < bones.Length - 1; i++) {
                    //boneMsg.Append($"\n[{i}] Length: {bones[i].length}, Read pos: {bones[i].readPosition}, solve pos: {bones[i].solverPosition}");
                    delta = (bones[i + 1].solverPosition - bones[i].solverPosition) * stretchingMlp;
                    bones[i + 1].solverPosition += mAdd + delta;
                    mAdd += delta;
                }*/
                
                float distanceToTarget = Vector3.Distance(bones[0].solverPosition, bones[^1].solverPosition);
                float stretchF = distanceToTarget / bodyLength;

                float m = stretchCurve.Evaluate(stretchF);
                
                for (int i = 0; i < bones.Length - 1; i++) {
                    //boneMsg.Append($"\n[{i}] Length: {bones[i].length}, Read pos: {bones[i].readPosition}, solve pos: {bones[i].solverPosition}");
                    delta = (bones[i + 1].solverPosition - bones[i].solverPosition) * m;

                    bones[i].solverPosition += delta;
                }
                
                //Debug.Log($"[SPINE STRETCHING] {boneMsg.ToString()}");
            }

            private void FABRIKPass(Vector3 animatedPelvisPos, Vector3 rootUp, float weight) {
                Vector3 startPos = Vector3.Lerp(pelvis.solverPosition, animatedPelvisPos, maintainPelvisPosition) +
                ...
                Vector3
                    startOffset =
                        Vector3.zero; // (bones[bones.Length - 1].solverPosition - bones[0].solverPosition) * weight;
            ...
            private void SolvePelvis() {
                ...
                if (pelvisPositionWeight > 0f) {
                    ...
                    Vector3 delta = ((IKPositionPelvis + pelvisPositionOffset) - pelvis.solverPosition) *
                    ...
                    if (hasChest && hasNeck) {
                        VirtualBone.SolveTrigonometric(bones, spineIndex, chestIndex, headIndex, headPosition,
                            bendNormal, pelvisPositionWeight * 0.9f);
            ...
            public void InverseTranslateToHead(Leg[] legs, bool limited, bool useCurrentLegMag, Vector3 offset,
                float w) {
                Vector3
                    delta = (headPosition + offset - head.solverPosition) *
                            w; // * (1f - pelvisPositionWeight); This makes the head lose its target when pelvisPositionWeight is between 0 and 1.
            ...
            private void TranslatePelvis(Leg[] legs, Vector3 deltaPosition, Quaternion deltaRotation, float scale) {
                ...
                MovePosition(LimitPelvisPosition(legs,
                    pelvis.solverPosition + deltaPosition * bodyPosStiffness * positionWeight, false));
            }
            ...
            private Vector3 LimitPelvisPosition(Leg[] legs, Vector3 pelvisPosition, bool useCurrentLegMag, int it = 2) {
                ...
                if (useCurrentLegMag) {
                    foreach (Leg leg in legs) {
                        leg.currentMag =
                            Mathf.Max(Vector3.Distance(leg.thigh.solverPosition, leg.lastBone.solverPosition),
                                leg.currentMag);
                    }
                }
